...
void gpioPinWrite(uint8_t ch, bool value)
{
  ...
  if (value == true)
    {
      HAL_GPIO_WritePin(gpio_tbl[ch].port, gpio_tbl[ch].pin, gpio_tbl[ch].onstate);
    }
  else
    {
      HAL_GPIO_WritePin(gpio_tbl[ch].port, gpio_tbl[ch].pin, gpio_tbl[ch].offstate);
    }
}
...
bool gpioPinRead(uint8_t ch)
{
  ...
  if (HAL_GPIO_ReadPin(gpio_tbl[ch].port, gpio_tbl[ch].pin) == gpio_tbl[ch].onstate)
    {
      ret= true;
    }
